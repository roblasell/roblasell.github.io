...
void makeFilters() {
  ...
  filterMenu.addFilter(filterX, currentHeight + (filterHeight * 2 * i++), filterHeight, filterHeight, "17 - 49", "17 - 49", "age");
  filterMenu.addFilter(filterX, currentHeight + (filterHeight * 2 * i++), filterHeight, filterHeight, "50+", "50+", "age");
  i++; // survived
...
void setupStoryButtons() {
  ...
  currentX += w + margin;
  stories[2] = new StoryButton(currentX, storyY1 + margin, w, storyBoxHeight - (margin * 2), 93, "of older 3rd class people died", victimHighlightColor);
  currentX += w + margin;
  ...
  // 3rd class old people
  stories[2].filters.add(filterMenu.filters.get(4));
  stories[2].filters.add(filterMenu.filters.get(8));
